...
public class OptitrackRigidBody : MonoBehaviour {	
	...
	public Vector3 scalePosition = new Vector3(1f,1f,1f);
		...
		if (foundIndex == false) {
			...
			if (usePostionTracking) {
				...
				if (originOverride != null) {
					...
					pos = (OptitrackRigidBodyManager.instance.rigidBodyPositions [index] - OptitrackRigidBodyManager.instance.origin.position) + originOverride.transform.position;
				} else {
					//transform.position = OptitrackRigidBodyManager.instance.rigidBodyPositions [index];
					pos = OptitrackRigidBodyManager.instance.rigidBodyPositions [index];
				}
				transform.position = new Vector3(pos.x * scalePosition.x, (pos.y * scalePosition.y) + floor, pos.z * scalePosition.z);
			}	
